...
from ...types.ca_telematics_risk_scores import batch_submit_params
from ...types.ca_telematics_risk_scores.batch_results_response import BatchResultsResponse
...
class BatchResource(SyncAPIResource):
    ...
    def results(
        ...
        *,
        ...
        return self._get(
            f"/v1/risk-score/ca-telematics/batch/{job_id}/results",
    ...
    def status(
        ...
        *,
        ...
        if not job_id:
            raise ValueError(f"Expected a non-empty value for `job_id` but received {job_id!r}")
        return self._get(
            f"/v1/risk-score/ca-telematics/batch/{job_id}/status",
            options=make_request_options(
                extra_headers=extra_headers, extra_query=extra_query, extra_body=extra_body, timeout=timeout
            ),
    ...
    def submit(
        self,
        *,
        body: Iterable[batch_submit_params.Body],
        # Use the following arguments if you need to pass additional parameters to the API that aren't available via kwargs.
        # The extra values given here take precedence over values defined on the client or passed to this method.
        extra_headers: Headers | None = None,
        extra_query: Query | None = None,
        ...
        """
        return self._post(
            "/v1/risk-score/ca-telematics/batch",
            body=maybe_transform(body, Iterable[batch_submit_params.Body]),
            options=make_request_options(
                extra_headers=extra_headers, extra_query=extra_query, extra_body=extra_body, timeout=timeout
            ),
            cast_to=BatchJobStatus,
    ...
    async def submit(
        ...
        *,
        ...
        return await self._post(
            "/v1/risk-score/ca-telematics/batch",
            body=await async_maybe_transform(body, Iterable[batch_submit_params.Body]),
            options=make_request_options(
                extra_headers=extra_headers, extra_query=extra_query, extra_body=extra_body, timeout=timeout
            ),
            cast_to=BatchJobStatus,
