...
from ...types.aggregated_crashes import batch_submit_params
from ...types.shared.batch_job_status import BatchJobStatus
from ...types.aggregated_crashes.batch_results_response import BatchResultsResponse
...
class BatchResource(SyncAPIResource):
    ...
    def results(
        ...
        *,
        ...
        return self._get(
            f"/v1/crash-data/aggregate/batch/{job_id}/results",
    ...
    def status(
        ...
        *,
        ...
        if not job_id:
            raise ValueError(f"Expected a non-empty value for `job_id` but received {job_id!r}")
        return self._get(
            f"/v1/crash-data/aggregate/batch/{job_id}/status",
            options=make_request_options(
                extra_headers=extra_headers, extra_query=extra_query, extra_body=extra_body, timeout=timeout
            ),
